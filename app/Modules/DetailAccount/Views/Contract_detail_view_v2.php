<?php 
        ...
        ?>
			<ul class="nav nav-tabs" id="myTab" role="tablist">
				...
				?>
						<li class="nav-item" id="contract_<?=$row["CM_CARD_NMBR"];?>" role="presentation">
							<button 
								class="nav-link <?=$is_active?>" 
								id="tab-<?= $row["CM_CARD_NMBR"] ?>" 
								data-bs-toggle="tab" 
								data-bs-target="#tab-pane-<?= $row["CM_CARD_NMBR"] ?>" 
								type="button" 
								role="tab" 
								aria-controls="tab-pane-<?= $row["CM_CARD_NMBR"] ?>" 
			...
			<!-- Profile -->
			<div class="tab-content" id="myTabContent" style="height: auto; color: black;">
				<?php 
				$i = 1;
			foreach($contracts as $row){
				$is_active = ($i == 1 ? "show active" : '');
			?>
				<div class="container mb-3 tab-pane fade <?=$is_active?>" id="tab-pane-<?=$row["CM_CARD_NMBR"];?>" role="tabpanel" aria-labelledby="tab-<?=$row["CM_CARD_NMBR"];?>" tabindex="0" style="height: 600px; overflow: auto;">
