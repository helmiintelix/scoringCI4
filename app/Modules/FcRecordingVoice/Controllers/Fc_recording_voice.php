...
class Fc_recording_voice extends \App\Controllers\BaseController
{
	...
	function get_path(){
		...
		$data["file_path"] = 'https://sbmobcoll.ecentrix.net/api/recording/stream/context/ecentrix-mobcoll/id/'.$data['id'].'/token/'.$this->Fc_recording_voice_model->get_ecx8_token();
		$rs = array('success' => true, 'message' => '', 'data' => $data);
		return $this->response->setStatusCode(200)->setJSON($rs);
	}
