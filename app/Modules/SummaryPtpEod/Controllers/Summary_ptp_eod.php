<?php 
namespace App\Modules\SummaryPtpEod\Controllers;
use App\Modules\SummaryPtpEod\Models\Summary_ptp_eod_model;
use CodeIgniter\Cookie\Cookie;
use DateTime;

class Summary_ptp_eod extends \App\Controllers\BaseController
{

	function __construct()
	{
		$this->Summary_ptp_eod_model = new Summary_ptp_eod_model();
	}
	...
	function index(){
		...
		// dd($data);
		return view('\App\Modules\SummaryPtpEod\Views\Report_sum_janji_bayar_group_kcu_view', $data);
	}
	function get_report_sum_janji_bayar_list(){
		$dataInput['tgl_from'] = '';
		$dataInput['tgl_to'] = '';
		...
		// print_r($dataInput);
		// exit();
		$data = $this->Summary_ptp_eod_model->get_report_sum_janji_bayar_list($dataInput);
	    if ($data) {
			$cacheKey = session()->get('USER_ID') . '_report_sum_janji_bayar_list';
