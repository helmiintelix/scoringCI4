...
jQuery(function($) {
	...
	var ci_controller = GLOBAL_MAIN_VARS["SITE_URL"] + "settings/deviation_approval_list";
	...
	jQuery(grid_selector).jqGrid({
		...
		height: 340,
		width: null,
		colNames:['ID','DEV APPROVAL ID','DEV APPROVAL NAME', 'DEV REF ID','APPROVAL 1 USER 1','APPROVAL 1 USER 2','APPROVAL 1 USER 3','APPROVAL 2 USER 1','APPROVAL 2 USER 2','APPROVAL 2 USER 3','APPROVAL 3 USER 1','APPROVAL 3 USER 2','APPROVAL 3 USER 3','APPROVAL 4 USER 1','APPROVAL 4 USER 2','APPROVAL 4 USER 3','CREATED_TIME','CREATED_BY'],
		colModel:[
      	...
      	return 'style="white-space: normal;' 
			...
			{name:'dev_ref_id', index:'dev_ref_id', width:160, cellattr: function (rowId, tv, rawObject, cm, rdata) { 
      	return 'style="white-space: normal;' 
			}},
		...
		altRows: true,
		caption:"DEVIATION APPROVAL",
		multiselect: false,
		onSelectRow: function(data){
			var arr = $(this).getRowData(data);
			arr_id = arr.dev_app_id;
		},
		toolbar: [true, "top"], 
...
jQuery(grid_selector).jqGrid('filterToolbar',{defaultSearch:true,stringResult:true});		
	...
	$("#t_deviation_approval-list-grid-table").append(
		...
		+"<button class='btn btn-sm btn-primary' id='btn-deviation_approval-edit'>EDIT</button>&nbsp;"
		+(GLOBAL_SESSION_VARS["LEVEL_GROUP"]=='ROOT'? "":"<button class='btn btn-sm btn-danger' id='btn-deviation_approval-delete'>DELETE</button>")
	);

	var grid_app_selector = "#deviation_approval-appr-list-grid-table";
	var pager_app_selector = "#deviation_approval-appr-list-grid-pager";
	var ci_app_controller = GLOBAL_MAIN_VARS["SITE_URL"] + "settings/deviation_approval_list_temp";
	...
	$("#btn-deviation_approval-add").click(function() {
		...
		showCommonDialog(900, 500, 'ADD DEVIATION APPROVAL', GLOBAL_MAIN_VARS["SITE_URL"] + 'settings/deviation_approval_add_form/', buttons);	
	});
		...
		if(selr)
		{
				...
				"success" :
				 {
					...
					"callback": function() {
						...
						var options = {
							url : GLOBAL_MAIN_VARS["SITE_URL"] + "settings/save_deviation_approval_edit/",
							type: "post",
							beforeSubmit: jqformValidate,
							success: showFormResponse,
							dataType: 'json',
			...
			showCommonDialog(900, 500, 'EDIT DEVIATION APPROVAL', GLOBAL_MAIN_VARS["SITE_URL"] + 'settings/deviation_approvalEditForm/' + selr, buttons);	
		} else {
	...
	$("#btn-deviation_approval-delete").click(function() {
		...
		if(selr)
		{
			bootbox.confirm("Apakah anda yakin akan menghapus?", function(result) {
				if(result) {
					$.post(GLOBAL_MAIN_VARS["SITE_URL"] + "settings/delete_deviation_approval/", { id: arr_id }, function(responseText) {
						if(responseText.success) {
							showInfo("Berhasil.");
							$(grid_selector).trigger('reloadGrid');
							$(grid_app_selector).trigger('reloadGrid');
