jQuery(function($) {
	var grid_appr_selector = "#mpa-appr-list-grid-table";
	var pager_appr_selector = "#mpa-appr-list-grid-pager";
	
	var ci_appr_controller = GLOBAL_MAIN_VARS["SITE_URL"] + "settings/mpa_list_temp";
	...
	var ci_controller = GLOBAL_MAIN_VARS["SITE_URL"] + "settings/mpa_list";
	...
	$("#t_mpa-list-grid-table").append(
				...
				+"<button class='btn btn-sm btn-primary' id='btn-mpa-edit'>EDIT</button>&nbsp;"
				//"<button class='btn btn-sm btn-danger' id='btn-mpa-delete'>DELETE</button>"
	);
				...
				"success" :
				 {
					...
					"callback": function() {
						...
						var options = {
							url : GLOBAL_MAIN_VARS["SITE_URL"] + "settings/save_mpa_edit/",
							type: "post",
							beforeSubmit: jqformValidate,
							success: showFormResponse,
							dataType: 'json',
...
{
	var passed = true;
	if(($("#txt-mpa-info").val()=="") || ($("#txt-mpa-dpd-from").val()=="") || ($("#txt-mpa-dpd-to").val()=="") ||(tinyMCE.activeEditor.getContent()=="")){
		passed = false;
	}
	if(($("#txt-mpa-dpd-from").val()*1) > ($("#txt-mpa-dpd-to").val()*1)){
		passed = false;
