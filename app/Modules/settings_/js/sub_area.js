jQuery(function($) {
	var grid_appr_selector = "#sub-area-appr-list-grid-table";
	var pager_appr_selector = "#sub-area-appr-list-grid-pager";
	
	var ci_appr_controller = GLOBAL_MAIN_VARS["SITE_URL"] + "settings/sub_area_list_temp";
...
jQuery(grid_appr_selector).jqGrid('filterToolbar',{defaultSearch:true,stringResult:true});	
	...
	var ci_controller = GLOBAL_MAIN_VARS["SITE_URL"] + "settings/sub_area_list";
	...
	jQuery(grid_selector).jqGrid({
		...
		height: "100%",
		width: null,
		colNames:['ID','KODE SUB AREA', 'NAMA SUB AREA','AREA NAME','ZIP CODE HANDLED','TGL INPUT','TGL UPDATE','STATUS AKTIF','STATUS APPROVAL'],
		colModel:[
			{name:'id',index:'a.id', width:60, hidden:true},
			{name:'sub_area_id',index:'a.sub_area_id', width:120},
			{name:'sub_area_name', index:'a.sub_area_name', width:160, sorttype:"int"},
...
jQuery(grid_selector).jqGrid('filterToolbar',{defaultSearch:true,stringResult:true});	
	...
	$("#t_sub-area-list-grid-table").append(
		...
		+ "<button class='btn btn-sm btn-primary' id='btn-sub-area-edit'>EDIT</button>&nbsp;"
	...
	$("#btn-sub-area-add").click(function() {
		...
		showCommonDialog(900, 500, 'ADD SUB AREA', GLOBAL_MAIN_VARS["SITE_URL"] + 'settings/sub_area_add_form/', buttons);	
	});
	...
	$("#btn-sub-area-edit").click(function() {
		...
		if(selr)
		{
				...
				"success" :
				 {
					...
					"callback": function() {
						...
						var options = {
							url : GLOBAL_MAIN_VARS["SITE_URL"] + "settings/save_sub_area_edit/",
							type: "post",
							beforeSubmit: jqformValidate,
							success: showFormResponse,
							dataType: 'json',
			...
			showCommonDialog(900, 500, 'EDIT SUB AREA', GLOBAL_MAIN_VARS["SITE_URL"] + 'settings/sub_area_edit_form/' + selr, buttons);	
		} else {
	...
	$("#btn-sub-area-delete").click(function() {
		...
		if(selr)
		{
			bootbox.confirm("Apakah anda yakin akan melakukan reject?", function(result) {
				if(result) {
					$.post(GLOBAL_MAIN_VARS["SITE_URL"] + "settings/delete_sub_area/", { id: selr }, function(responseText) {
						if(responseText.success) {
							showInfo("Berhasil.");
							$(grid_selector).trigger('reloadGrid');
							$(grid_appr_selector).trigger('reloadGrid');
