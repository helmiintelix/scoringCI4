...
jQuery(function($) {
	...
	var ci_controller = GLOBAL_MAIN_VARS["SITE_URL"] + "settings/kcu_list_temp";
	...
	jQuery(grid_selector).jqGrid({
		...
		height: 340,
		width: null,
		colNames:['ID KCU', 'ACTION','KODE COLLECTION CENTER', 'NAMA COLLECTION CENTER','ZIP CODE','ALAMAT','TGL INPUT','TGL UPDATE','STATUS AKTIF','STATUS APPROVAL'],
		colModel:[
			{name:'id',index:'id', width:60, hidden:true},
			{name:'action', index:'action', width:200},
			{name:'kcu_id', index:'kcu_id', width:180},
			{name:'kcu_name', index:'kcu_name', width:200},
			{name:'zip_code_list', index:'zip_code_list', width:300},
	...
	$("#t_kcu-list-grid-table").append(
		...
		"<button class='btn btn-sm btn-primary' id='btn-kcu-edit'>APPROVE</button>&nbsp;" +
		"<button class='btn btn-sm btn-danger' id='btn-kcu-delete'>REJECT</button>"
	);
...
/*		var buttons = {
		...
		showCommonDialog(900, 500, 'ADD KCU GROUP', GLOBAL_MAIN_VARS["SITE_URL"] + 'settings/kcu_add_form/', buttons);	
*/
...
/*			var buttons = {
				"success" :
				 {
					...
					"callback": function() {
						...
						var options = {
							url : GLOBAL_MAIN_VARS["SITE_URL"] + "settings/save_kcu_edit_temp/",
							type: "post",
							beforeSubmit: jqformValidate,
							success: showFormResponse,
							dataType: 'json',
			...
			showCommonDialog(900, 500, 'EDIT KCU TEMPLATE', GLOBAL_MAIN_VARS["SITE_URL"] + 'settings/kcuEditForm_temp/' + selr, buttons);	
*/
			bootbox.confirm("Apakah anda yakin akan melakukan approval?", function(result) {
				if(result) {
					$.post(GLOBAL_MAIN_VARS["SITE_URL"] + "settings/save_kcu_edit_temp/", { id: arr_id, arr_kk : arr_kk }, function(responseText) {
						if(responseText.success) {
							showInfo("Berhasil.");
							$(grid_selector).trigger('reloadGrid');
						} else{
		...
		if(selr)
		{
			bootbox.confirm("Apakah anda yakin akan melakukan reject?", function(result) {
				if(result) {
					$.post(GLOBAL_MAIN_VARS["SITE_URL"] + "settings/delete_kcu_office/", { id: arr_id, arr_kk: arr_kk }, function(responseText) {
						if(responseText.success) {
							showInfo("Berhasil.");
							$(grid_selector).trigger('reloadGrid');
						} else{
		...
		if(selr)
		{
				...
				"success" :
				 {
					...
					"callback": function() {
						...
						var options = {
							url : GLOBAL_MAIN_VARS["SITE_URL"] + "settings/save_note_reject_kcu/",
							type: "post",
							beforeSubmit: jqformValidate,
							success: showFormResponse,
							dataType: 'json',
			...
			showCommonDialog(900, 500, 'EDIT KCU TEMPLATE', GLOBAL_MAIN_VARS["SITE_URL"] + 'settings/note_reject_kcu/' + selr, buttons);	
			*/

			bootbox.confirm("Apakah anda yakin akan melakukan reject?", function(result) {
				if(result) {
					$.post(GLOBAL_MAIN_VARS["SITE_URL"] + "settings/save_note_reject_kcu/", {arr_kk: arr_kk}, function(responseText) {
						if(responseText.success) {
							showInfo("Berhasil.");
							$(grid_selector).trigger('reloadGrid');
						} else{
	...
	$("#btn-kcu-delete").click(function() {
		...
		if(selr)
		{
			bootbox.confirm("Are you sure you want to delete this kcu content?", function(result) {
				if(result) {
					$.post(GLOBAL_MAIN_VARS["SITE_URL"] + "settings/delete_kcu/", { id: selr }, function(responseText) {
						if(responseText.success) {
							showInfo("Berhasil.");
							$(grid_selector).trigger('reloadGrid');
						} else{
...
{	
	var passed = true;
	if(($("#txt-kcu-id").val()=="") || ($("#txt-kcu-name").val()=="")){
		passed = false;
	}
	if(!passed) {
