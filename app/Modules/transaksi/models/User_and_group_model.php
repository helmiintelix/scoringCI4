...
class User_and_group_model extends CI_model
{
    ...
    function get_user_group_management_list()
    {
        $select = array('a.id', 'a.name', 'a.description', 'a.level_group', 'IF(a.is_active = "1", "ENABLE", "DISABLE") AS flag,a.type_collection', 'DATE_FORMAT(a.created_time,"%d-%b-%Y") created_time', 'DATE_FORMAT(a.updated_time,"%d-%b-%Y") updated_time');
        ...
        foreach ($rResult->list_fields() as $key => $value) {
    ...
    function get_user_group_management_list_temp()
    {
        $select = array('a.id', 'a.name', 'a.description', 'a.level_group', 'IF(a.is_active = "1", "ENABLE", "DISABLE") AS flag, a.notes,a.type_collection', 'DATE_FORMAT(a.created_time,"%d-%b-%Y") created_time', 'DATE_FORMAT(a.updated_time,"%d-%b-%Y") updated_time');
...
*/
    ...
    function save_user_group_edit_temp($user_data)
    {
        ...
        $sql = "SELECT * FROM cc_user where group_id = '" . $user_data["id"] . "' ";
        $result = $this->db->query($sql)->result_array();

        foreach ($result as $a => $b) {
            $this->update_acs_user($b["id"]);
        }
    ...
    function get_user_management_list()
    {
        ...
        $select = array(
            ...
            // 'IF(a.failed_attempt < "' . $max_failed . '", concat("",a.failed_attempt,""),concat("<span class=\"label label-sm label-warning\">",a.failed_attempt,"")) failed_attempt',
            'a.supervisor_name',
            'a.type_collection',
            'a.email',
            'a.handphone',
    ...
    function get_user_management_list_temp()
    {
        ...
        $select = array(
            ...
            'IF(a.flag_status = "1", "Approve", IF(a.flag_status = "0", "No Status", "Reject")) AS is_status',
            // 'IF(a.failed_attempt < "' . $max_failed . '", concat("<span class=\"label label-sm label-info\">",failed_attempt,"</span>"),concat("<span class=\"label label-sm label-warning\">",failed_attempt,"</span>")) failed_attempt',
            'a.notes',
            'a.email',
            'a.handphone',
            'a.supervisor_name',
        ...
        $this->db->join('cc_user b','a.created_by=b.id','left');
        $this->db->join('cc_user c','a.updated_by=c.id','left');
        $this->db->join('cc_user_group d','a.group_id=d.id');
        $this->db->where('a.is_active <> "2"');
        $this->db->where('a.flag_status', NULL);
        $where2 = "a.flag = '0'";
    ...
    // 				// if($fieldName=='login_status_tele'){
    // 				// $this->tele->query("SET SESSION group_concat_max_len = 1000000;");
    // 				// $sqlx = 'select group_CONCAT("\'",id,"\'") id from acs_user where is_login = ? ';
    // 				// $rs = $this->tele->query($sqlx,array($fieldData))->result_array()[0]['id'];
    // 				// // echo $this->tele->last_query();

    // 				// if($rs!=''){
    // 				// $fieldOperation = 'a.id in ('.$rs.')';
    // 				// }else{
    // 				// $fieldOperation = 'a.id in ("")';
    // 				// }
    ...
    // 		'DATE_FORMAT(a.created_time,"%d-%b-%Y") created_time',
    // 		'DATE_FORMAT(a.updated_time,"%d-%b-%Y") updated_time',
    // 		'concat(c.branch_id," - ",c.branch_name) as group_kcu',
    // 		'a.id',
    // 		'a.flag',
    // 		'a.name',
    // 		'd.name as group_id',
    ...
    // 		'IF(a.failed_attempt < "' . $max_failed . '", concat("<span class=\"label label-sm label-info\">",a.failed_attempt,"</span>"),concat("<span class=\"label label-sm label-warning\">",a.failed_attempt,"</span>")) failed_attempt',
    // 		'rto.name report_to',
    // 		'a.nik',
    // 		'a.address',
    // 		'a.supervisor_name',
    ...
    // 		// $id = 	$list[$row_number]["cell"]["id"];
    // 		// $sql = "SELECT COUNT(*) jumlah FROM cc_user where id = ? ";
    // 		// $jumlah = $this->db->query($sql , array($id))->result_array()[0]['jumlah'];
    // 		// if($jumlah>0){
    // 		// $sql = "SELECT is_login FROM cc_user where id = ? ";
    // 		// $is_login = $this->$sql->get();
    // 		// $this->db->query($sql , array($id))->result_array()[0]['is_login'];

    // 		// if($is_login=='1'){
    // 		// $list[$row_number]["cell"]["login_status_tele"] = "<span class=\"label label-sm label-success\">Logged in</span>";
    // 		// }else{
    // 		// $list[$row_number]["cell"]["login_status_tele"] = "<span class=\"label label-sm label-danger\">Logged out</span>";
    // 		// }
    ...
    // 	$select = array(
    // 		'"" AS list_number', 'a.id', 'a.name', 'a.flag',
    // 		'DATE_FORMAT(a.created_time,"%d-%b-%Y") created_time',
    // 		'DATE_FORMAT(a.updated_time,"%d-%b-%Y") updated_time',
    // 		"a.kcu_list as kcu_office",
    // 		'd.name as group_id',
    ...
    function save_user_edit($user_data)
    {
        ...
        $return = $this->db->replace('cc_user_tmp', $user_data);
    ...
    function save_user_edit_temp($user_data,$status)
    {
        ...
        $this->db->from('cc_user_tmp');
        $this->db->where('id', $user_data["id"]);
        $this->db->where('flag_status', null);
        $hasil = $this->db->get();
        ...
        if ($hasil->num_rows() > 0) {
            ...
            $this->db->where('id', $user_data["id"]);
            $this->db->from('cc_user_tmp');
            $user_get = $this->db->get();
            $user = $user_get->result_array();
            
            $user = $user_get->result_array();
            $this->db->insert('cc_user', $user[0]);
            
            $user[0] = $user[0] + array('status'=>$status , 'insert_by'=>$this->session->userdata('USER_ID'),'insert_time'=>date('Y-m-d H:i:s'));
            $this->db->insert('cc_user_tmp_history', $user[0]);
    ...
    function get_export_list_user()
    {
        ...
        $select = array(
            'a.id as UserID',
            'a.name as Name',
            'spv.name as SupervisorName',
            'a.created_time CreationDate',
            'a.group_id as Role',
            'a.email',
            'a.handphone'
        );
    ...
    function update_acs_user($id)
    {
        ...
        $group_level = $this->common_model->get_record_value("level_group", "cc_user_group", "id = '" . $group_id . "' ", "");
        ...
        // if($tipe=="TeleColl"){
        $profile = '';
        if ($group_level == "FIELD_COLL" || $group_level == "TELECOLL") {
            $group_id = 'AGENT';
            $profile = 'AGENT';
        } else if ($group_level == "ADMIN") {
            $group_id = 'ADMIN';
            $profile = 'ADMIN';
        } else if ($group_level == "MANAGER") {
            $group_id = 'MANAGER';
            $profile = 'COLLECTION_HEAD';
        } else if ($group_level == "TEAM_LEADER") {

            $group_id = 'COORDINATOR';
            $profile = 'COORDINATOR_TYPE_A';
        } else if ($group_level == "RECOVERY") { //IT SECURITY

            $group_id = 'ADMIN';
            $profile = 'COORDINATOR_TYPE_B';
        } else if ($group_level == "SUPERVISOR") { //IT APP

            $group_id = 'ADMIN';
            $profile = 'COORDINATOR_TYPE_C';
        }
        ...
        if ($profile != '') {
            ...
            $sql = "REPLACE ecentrix_agent
							SELECT '" . $user_data["id"] . "' , '" . $name . "' , 'predictive1' , now()   ";
            $this->cti->query($sql);
        }
    }
}
