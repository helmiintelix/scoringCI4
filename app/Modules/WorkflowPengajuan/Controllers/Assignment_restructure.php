...
use App\Modules\WorkflowPengajuan\Models\Assignment_restructure_model;
use App\Modules\WorkflowPengajuan\Models\Payment_plan_model;
use App\Modules\WorkflowPengajuan\Models\Anuitas_model;
use CodeIgniter\Cookie\Cookie;


class Assignment_restructure extends \App\Controllers\BaseController
{
	...
	function __construct()
	{
		...
		$this->Assignment_restructure_model = new Assignment_restructure_model();
	...
	function index(){
		...
		return view('\App\Modules\WorkflowPengajuan\Views\Workflow_pengajuan_restructure_list_view', $data);
	}
	function workflow_pengajuan_restructure_list(){
		$statusOld = "";
		$data['status'] = $this->input->getGet('status');
		$cache = session()->get('USER_ID').'_workflow_pengajuan_restructure_list';
		if($data['status'] != $statusOld){
			$this->cache->delete($cache);
			
			$data = $this->Assignment_restructure_model->workflow_pengajuan_restructure_list($data['status']);
	...
	function requestData(){
		return view('\App\Modules\WorkflowPengajuan\Views\Search_account_restructure_view');
	}
	...
	function pengajuan_program_form() {
		...
		$data['deviation_reason'] = array(''=>'[select data]') + $this->Common_model->get_ref_master('deviation_id value , deviation_name item', 'cms_deviation_reference', 'is_active="1" and type like "%RESTRUCTURE%"  ', 'deviation_name asc');
		...
		$data['kondisi_khusus_list'] = $this->Common_model->get_ref_master('value , CONCAT( value , " - " , description) item', 'cms_reference', 'reference="KONDISI_KHUSUS_PL_RSTR" and flag="1" and flag_tmp="1" ', 'order_num asc');

		$data['grace_period'] = $this->get_tanggal_ptp_grace($data["credit_data"]['CM_BUCKET']);
		$data["data_pengajuan"] = $this->Common_model->get_record_values("*", "cms_pengajuan_restructure ", "id='".$id."'");
		...
		// exit();
		// dd($data);
		return view('\App\Modules\WorkflowPengajuan\Views\Detail_contract_request_program_restructure_view', $data)
		.view('\App\Modules\WorkflowPengajuan\Views\Request_pengajuan_restructure_view', $data);

		// $this->load->view('detail_contract_request_program_restructure_view',$data);
		
		// $this->load->view('request_pengajuan_restructure_view', $data);		
	}
	...
	function get_data_kotak_merah(){
		...
		$total_due_balance_installemnt = (int) $this->Common_model->get_record_value("CM_TOT_PRINCIPAL", "cpcrd_new", " CM_CARD_NMBR ='". $agreement_no ."'");
		...
		// late_charge
		$late_charge =  $this->Common_model->get_record_value('CM_RTL_MISC_FEES','cpcrd_new','CM_CARD_NMBR = "'.$agreement_no.'" ');
	...
	function cek_config_rest(){
		...
		$cek_parameter_tmp = $this->Common_model->get_record_value('id_pengajuan','cms_restructure_parameter_tmp','id_pengajuan="'.$id_pengajuan .'"');
		if($cek_parameter_tmp != ''){
			
			$diskon_parameter = $this->Common_model->get_record_value('restructure_parameter_id','cms_restructure_parameter_tmp','id_pengajuan="'.$id_pengajuan .'"');
			$builder = $this->db->table("cms_restructure_parameter_tmp");
			$builder->select("*");
			...
			$data	= array("success" => true, "message" => "Success" , "restructure_parameter"=> $diskon_parameter , 'data'=>$data_diskon ,'alert'=>$message);
		...
		$data	= array("success" => false, "message" => "failed" , "restructure_parameter"=> 'NOT_FOUND' , 'data'=>'NOT FOUND', 'alert'=> 'NOT FOUND');
		...
		if($flag_kondisi_khusus == 0 || $flag_kondisi_khusus == '0'){ //block code
			$builder = $this->db->table("cms_restructure_parameter");
			$builder->select("*");
			$builder->where("hirarki", "2");
			$builder->where("is_active", "1");
			$builder->where("restructure_tipe", $request_type);
			$builder->where("bucket_list !=", null);
			...
			if(count($result) >0 ){
				foreach ($result as $key => $value) {
					...
					$builder->where($value['restructure_parameter_detail']);
					...
					if($jumlah>0){
						$diskon_parameter = $value['restructure_parameter_id'];
						$diskon_parameter_name = $value['restructure_parameter_name'];
						$builder = $this->db->table('cms_restructure_parameter');
						$builder->where('restructure_parameter_id', $diskon_parameter);
						$builder->where('is_active', '1');
						...
						$message = '* parameter BLOCK CODE untuk '.$agreement_no.' tidak ditemukan ';
					}
				}
				$data	= array("success" => true, "message" => "Success" , "restructure_parameter"=> $diskon_parameter , 'data'=>$data_diskon ,'alert'=>$message);
			}else{
		...
		else if($flag_kondisi_khusus != 0 && $flag_kondisi_khusus != '0' ) //kondisi khusus
		{
			
			if($desc_kondisi_khusus != ''){
				$builder = $this->db->table("cms_restructure_parameter");
				$builder->select("*");
				$builder->where('hirarki', '1');
				$builder->where('is_active', '1');
				$builder->where('restructure_tipe', $request_type);
				$builder->like('desc_kondisi_khusus', $desc_kondisi_khusus);
				$builder->where('bucket_list !=', 'null');
				$builder->orderBy('updated_time', 'desc');
				...
				if(count($result) >0 ){
					...
					foreach ($result as $key => $value) {
						...
						if($jumlah>0){
							...
							$message = '* parameter KONDISI KHUSUS untuk  '.$agreement_no.' tidak ditemukan ';
						}
					}
					$data	= array("success" => true, "message" => "Success" , "restructure_parameter"=> $diskon_parameter , 'data'=>$data_diskon,'alert'=>$message);
				}else{
					...
					$message = '* parameter KONDISI KHUSUS untuk  '.$agreement_no.' tidak ditemukan ';
					$data	= array("success" => true, "message" => "Success" , "restructure_parameter"=> "NOT_FOUND" , 'data'=>"NOT_FOUND",'alert'=>$message);
				}
			}else{
				
				$message = '* deskripsi kondisi khusus belum di pilih ';
				$data	= array("success" => true, "message" => "Success" , "restructure_parameter"=> "NOT_FOUND" , 'data'=>"NOT_FOUND",'alert'=>$message);
			}
		}
		

		if($data_diskon=='NOT_FOUND' && $flag_kondisi_khusus == '0'){ //normal
			$builder = $this->db->table("cms_restructure_parameter");
			$builder->select("*");
			$builder->where('hirarki', '3');
			$builder->where('is_active', '1');
			$builder->where('restructure_tipe', $request_type);
				...
				if(count($result) >0 ){
					foreach ($result as $key => $value) {
						...
						if($jumlah>0){
							...
							$query = $builder->get();
							$data_diskon = $query->getRowArray();
							// $message .= ", parameter  <b>NORMAL</b> digunakan ";
							break;  
	...
	function save_pengajuan_restructure(){
		// echo "success";
		...
		$data['restructure_rate_id']	 = $this->input->getPost('restructure_rate');
		...
		$data['tipe_pengajuan']	 = 'RSTR';
		$data['source']	 = $source;

		$builder = $this->db->table('cms_pengajuan_restructure');
		if(trim($data['id']??'')==''){
			$data['id']	= UUID(false);
		...
		foreach ($output as $key => $value) {
			...
			$data_payment_plan['relief_program'] = 'RESTRUCTURE';
		...
		$builder = $this->db->table('cms_restructure_parameter a');
		$builder->select("'{$data['id']}' as id_pengajuan, a.*");
		$builder->where('a.restructure_parameter_id', $data['restructure_rate_id']);
		$insertData = $builder->get()->getResultArray();
		if (!empty($insertData)) {
			$tmpBuilder = $this->db->table('cms_restructure_parameter_tmp');
		...
		$builder = $this->db->table('cms_restructure_parameter_tmp a');
		...
		if($this->input->getPost('status')=='1'){ //save and finish
			if(session()->get('LEVEL_GROUP')=='TELECOLL' || session()->get('LEVEL_GROUP')=='FIELD_COLL'){
				...
				$return_msg = $this->send_message("Request Restructure",$data['cm_card_nmbr'],'Email',array('id_pengajuan'=>$data['id']),$agent_id,'Internal');
			}else{
				$return_msg = true;
			}
		...
		if($result==true){
			$description = array(
				"action" => "RESTRUCTURE PROGRAM REQUEST", 
				"before" => null, 
				"after" => $this->Common_model->get_record_values("
					cm_card_nmbr as card_number,
					product_id as product_id,
					source,
					call_verification_status as call_verification_status,
					document_upload_status as document_upload_status,
					status as request_status", 
					'cms_pengajuan_restructure', 
					"id = '".$data['id']."'",""),
				"status" => "waiting verification", 
				"approval" => null);				
			$description = json_encode($description);
			$this->Common_model->data_logging('PROGRAM REQUEST', 'RESTRUCTURE PROGRAM REQUEST', 'SUCCESS', $description);
			$data	= array("success" => true, "message" => "Pengajuan Restructure Tersimpan" , 'id'=> $data['id'] );
		}else{
	...
	function save_contractdetail($id){
		$table_detail = 'cms_pengajuan_restructure_detail';
		$table_master = 'cms_pengajuan_restructure';

		$builder = $this->db->table('cpcrd_new a');
		$builder->select('
			b.id as id_pengajuan,
			a.CM_ORG_NMBR as CM_ORG_NMBR,
			a.CM_TYPE as CM_TYPE,
			a.CM_CARD_NMBR as CM_CARD_NMBR,
			a.CM_CUSTOMER_NMBR as CM_CUSTOMER_NMBR,
			a.CM_STATUS as CM_STATUS,
			a.CM_DTE_PYMT_DUE as CM_DTE_PYMT_DUE,
			a.CM_30DAYS_DELQ as CM_30DAYS_DELQ,
			a.CM_60DAYS_DELQ as CM_60DAYS_DELQ,
			a.CM_90DAYS_DELQ as CM_90DAYS_DELQ,
			a.CM_120DAYS_DELQ as CM_120DAYS_DELQ,
			a.CM_150DAYS_DELQ as CM_150DAYS_DELQ,
			a.CM_180DAYS_DELQ as CM_180DAYS_DELQ,
			a.CM_210DAYS_DELQ as CM_210DAYS_DELQ,
			a.CM_CRLIMIT as CM_CRLIMIT,
			a.CM_TOT_BALANCE as CM_TOT_BALANCE,
			a.CM_BLOCK_CODE as CM_BLOCK_CODE,
			a.CR_NAME_1 as CR_NAME_1,
			a.CR_EU_SEX as CR_EU_SEX,
			a.CM_CYCLE as CM_CYCLE,
			a.CM_BUCKET as CM_BUCKET,
			a.AGENT_ID as AGENT_ID,
			a.DPD as DPD,
			a.PRODUCT_ID as PRODUCT_ID,
			a.CM_INSTALLMENT_AMOUNT as CM_INSTALLMENT_AMOUNT,
			a.CM_PAID_CHARGE as CM_PAID_CHARGE,
			a.CM_OFFICER_NAME as CM_OFFICER_NAME
		');
	...
	function saveAssignData(){
		...
		$jumlah	= $this->Common_model->get_record_value("count(*)", "cms_pengajuan_restructure", "id = '".$id."' and status='NEW' ");
		if($jumlah>0){
			$jumlah	= $this->Common_model->get_record_value("count(*)", "cc_user", "id = '".$tl."' and is_active='1' ");
			...
			if($jumlah>0){
				...
				if($rs){
					$cache = session()->get('USER_ID').'_workflow_pengajuan_restructure_list';
					$this->cache->delete($cache);
					$response = array('success'=>true , 'message'=>'Restructure assign success');
				}else{
					$response = array('success'=>false , 'message'=>'Restructure assign failed');
				}
			}else{
			...
			$response = array('success'=>false , 'message'=>'Restructure not Found');
		}
	...
	function approvalrequest(){
		...
		$data["credit_data"] = $this->Common_model->get_record_values("*", "cms_pengajuan_restructure_detail ", "id_pengajuan='".$id."'");
		$data['deviation_reason'] = array(''=>'[select data]') + $this->Common_model->get_ref_master('deviation_id value , deviation_name item', 'cms_deviation_reference', 'is_active="1" and type like "%RESTRUCTURE%" and product like "%'.$data["credit_data"]['CM_TYPE'].'%"  ', 'deviation_name asc');
		...
		$select = array("a.id", "a.id_pengajuan" ,"b.description call_status" , "c.description call_result" , "d.name name" , "date(a.created_time) date" , "a.notes" , "time(a.created_time) time",
		"'' recording",
		"'-' action",
					"'' no",
					"a.created_time"
		...
		$builder->join('setup_approval_restructure_detail b', 'a.id=b.id_approval');
		// $builder->where($data["data_pengajuan"]['total'], '>= a.amount_from');
		$builder->where('a.amount_from <= ', $data["data_pengajuan"]['total']);
		$builder->where('a.amount_until >= ', $data["data_pengajuan"]['total']);
		// $builder->where($data["data_pengajuan"]['total'], '<= a.amount_until');
		...
		// 		WHERE '".$data["data_pengajuan"]['total']."' >= a.amount_from AND '".$data["data_pengajuan"]['total']."' <= a.amount_until 
		...
		if(count($result)>0){
			...
			$subquery1 = $this->db->table('setup_approval_restructure a')
			->select('a.id as id, c.id as user_id, c.name as name, 0 as urut')
			->join('setup_approval_restructure_detail b', 'a.id = b.id_approval')
			->join('cc_user c', 'b.user_id = c.id')
			->where('b.level_num', $value['level_num'])
			->where('a.amount_from <= ', $data["data_pengajuan"]['total'])
			->where('a.amount_until >= ', $data["data_pengajuan"]['total'])
			->orderBy('b.level_num')
			->orderBy('b.order_num')
			->getCompiledSelect();
			...
			->where('s.id_pengajuan', $data["data_pengajuan"]['id'])
			->getCompiledSelect();
		...
		// dd($data);
		return view('\App\Modules\WorkflowPengajuan\Views\Detail_contract_request_program_restructure_view', $data)
		.view('\App\Modules\WorkflowPengajuan\Views\Approval_pengajuan_restructure_view', $data);
		// $this->load->view('detail_contract_request_program_restructure_view',$data);
		// $this->load->view('approval_pengajuan_restructure_view',$data);
	}
	...
	function saveApprovalberjenjang (){
		...
		if($data['tipe_pengajuan']=="DP"){
			...
			$table = "cms_pengajuan_restructure";
			$table_approval = "setup_approval_restructure";
		}
		//add log
		$data_log['before']=$this->Common_model->get_record_values("
		cm_card_nmbr 'card number',
		...
		if($data['status']=="APPROVE"){
			...
			$numOfLevel = $this->Common_model->get_record_value('id', 'setup_approval_restructure', "id = '".$approval_id."'","");
			if($numOfLevel==$approval_id){
				$builder = $this->db->table($table);
				$builder->set('status', 'APPROVE');
				$builder->set('updated_by', $data['created_by']);
				$builder->set('updated_time', date('Y-m-d H:i:s'));
				$builder->where('id', $data['id_pengajuan']);
