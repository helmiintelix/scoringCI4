<?php 
namespace App\Modules\ReportPengajuanRestructure\Controllers;
use App\Modules\ReportPengajuanRestructure\Models\Report_pengajuan_restructure_model;
use CodeIgniter\Config\Services;
use CodeIgniter\Cookie\Cookie;


class Report_pengajuan_restructure extends \App\Controllers\BaseController
{

	function __construct()
	{
		$this->Report_pengajuan_restructure_model = new Report_pengajuan_restructure_model();
	}

	function index(){
		$data['classification'] = $this->input->getPost('classification');
		return view('\App\Modules\ReportPengajuanRestructure\Views\Report_pengajuan_restructure_view', $data);
	}
	function get_report_pengajuan_restructure(){
		$cache = session()->get('USER_ID').'_report_pengajuan_restructure';
		if($this->cache->get($cache)){
			$data = json_decode($this->cache->get($cache));
			$rs = array('success' => true, 'message' => '', 'data' => $data);
		}else{
			$data = $this->Report_pengajuan_restructure_model->get_report_pengajuan_restructure();
	...
	function view_restructure_form(){
		...
		$data["pengajuan_data"]	= $this->Common_model->get_record_values("*", "cms_pengajuan_restructure", "id='" . $data['id_pengajuan'] . "'");
		$data["parameter_diskon"]	= $this->Common_model->get_record_values("*", "cms_restructure_parameter_tmp", "id_pengajuan='" . $data['id_pengajuan'] . "'");



		$data['kondisi_khusus_list'] = $this->Common_model->get_ref_master('value , CONCAT( value , " - " , description) item', 'cms_reference', 'reference="KONDISI_KHUSUS_' . $product_id . '_' . $data['tipe_pengajuan'] . '" and flag="1" and flag_tmp="1" ', 'order_num asc');
		...
		$flag_verif_call	= $this->Common_model->get_record_value("call_verification_status", "cms_pengajuan_restructure", "id='" . $data['id_pengajuan'] . "'");
		$flag_verif_upload	= $this->Common_model->get_record_value("document_upload_status", "cms_pengajuan_restructure", "id='" . $data['id_pengajuan'] . "'");
		...
		$verif_call_checker =  $this->Common_model->get_record_value("call_verification_status", "cms_pengajuan_restructure", " id='" . $data['id_pengajuan'] . "' ");
		...
		} else if ($product_id == 'CC') {
			$data['deviation_reason'] = $this->Common_model->get_ref_master('deviation_id value , deviation_name item', 'cms_deviation_reference', 'is_active="1" and product LIKE "%CC%" ', 'deviation_name asc');
		}

		$flagDev = $this->Common_model->get_record_value("deviation_flag", "cms_pengajuan_restructure", "id='" . $data['id_pengajuan'] . "' ");

		$builder = $this->db->table("cms_approval a");
		if ($flagDev == '1') {
			$builder->select("a.approval_level, 
					if(a.`action`='REQUEST','Request By',
						if(a.`action`='VERIFICATION','Verified By',
							if(a.`action`='CHECKER','Checker By',
								if(a.`action`='APPROVAL_DEVIATION',CONCAT('Aproval ',a.approval_level),'')
							)
						)
					)as action,
					a.`status`, b.name, date(a.created_time)  created_time");
			$builder->join("cc_user b", "a.created_by=b.id");
			$builder->where("a.id_pengajuan", $data['id_pengajuan']);
			$builder->where("a.status", "APPROVE");
			$builder->where("a.tipe_pengajuan", "DV");
			$builder->orderBy("a.approval_level", "asc");
			$builder->orderBy("a.created_time", "asc");
			...
			$builder->where("a.id_pengajuan", $data['id_pengajuan']);
			$builder->where("a.status", "APPROVE");
			$builder->where("a.tipe_pengajuan", "RSTR");
			$builder->where("a.`action` <> 'APPROVAL_DEVIATION'");
			$builder->orderBy("a.approval_level", "asc");
			$builder->orderBy("a.created_time", "asc");
		}
		$data['arr_approval'] = $builder->get()->getResultArray();

		$data['USER_ID'] = session()->get('USER_ID');

		dd($data);
		if ($data['tipe_pengajuan'] == "RSTR") {
			return view('\App\Modules\ReportPengajuanRestructure\Views\Detail_pengajuan_restructure_view', $data);
		} else {
			return view('\App\Modules\ReportPengajuanRestructure\Views\Detail_pengajuan_reschedule_view', $data);
		}
	}
