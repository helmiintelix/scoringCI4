...
<script>
	function validateForm() {
		var x = document.forms["form_edit_group"]["txt-group-id"].value;
		if (x == "test") {
			alert("isinya test");
			return false;
		}
...
<form role="form" class="needs-validation" id="form_edit_group" name="form_edit_group" onsubmit="return validateForm()" novalidate method="post">
	<div>
		<label for="form-field-select-2">ID</label>
		<input type="hidden" id="txt-group-id" name="txt-group-id" class="form-control" value="<?= $user_data["id"] ?>" required readonly/>
		<input type="text" id="txt-group-idx" name="txt-group-idx" class="form-control" value="<?= $user_data["id"] ?>" required disabled/>
	</div>
	<br />
	<div>
		<label for="form-field-select-2">USER GROUP NAME</label>
		<input type="text" id="txt-group-name" name="txt-group-name" class="form-control" value="<?= $user_data["name"] ?>" required />
	</div>
	<br />

	<div>
		<label for="form-field-select-2">DESCRIPTION</label>
		<input type="text" id="txt-group-description" name="txt-group-description" value="<?= $user_data["description"] ?>" class="form-control" />
	</div>
	<br />
	...
	<div>
		...
		echo form_dropdown('opt-heirarki', $level_master, $user_data['level'], $attributes);
		?>
	</div>
	<br />
    <div>
		...
		<div class="row">
			<div class="col-md-12">
				<div class="card common-table-card mb-4">
					...
					<div class="card-body">
						<input type="hidden" id="txt-current-authority" name="txt-current-authority" value="<?= $user_data["menu_list"] ?>">
...
<script type="text/javascript">
	jQuery(function($) {
		...
		var txtAuthority = $("#txt-current-authority").val();
		var arrAuthority = txtAuthority.split("|");

		for (var i = 0; i < arrAuthority.length; i++) {
			$("#" + arrAuthority[i]).prop("checked", true);
			//alert("#" + arrAuthority[i]);
		}

		if(action=='VIEW'){
			$("#form_edit_group input").attr('disabled',true);
			$("#form_edit_group select").attr('disabled',true);
		}
	});
