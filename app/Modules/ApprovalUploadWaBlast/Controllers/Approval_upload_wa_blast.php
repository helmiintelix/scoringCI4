<?php 
namespace App\Modules\ApprovalUploadWaBlast\Controllers;
use App\Modules\ApprovalUploadWaBlast\Models\Approval_upload_wa_blast_model;
use PhpOffice\PhpSpreadsheet\IOFactory;
use CodeIgniter\Cookie\Cookie;
use DateTime;

class Approval_upload_wa_blast extends \App\Controllers\BaseController
{

	function __construct()
	{
		$this->Approval_upload_wa_blast_model = new Approval_upload_wa_blast_model();
	}

	function view_approval_upload_wa_blast_list(){
		$data['classification'] = $this->input->getPost('classification');
		return view('\App\Modules\ApprovalUploadWaBlast\Views\Approval_upload_wa_blast_view', $data);
	}
	function get_pengiriman_surat_file_list(){

		$data = $this->Approval_upload_wa_blast_model->get_pengiriman_surat_file_list();
	...
	function upload_file_form(){
		...
		return view('\App\Modules\ApprovalUploadWaBlast\Views\Upload_file_form_view',$data);
	}
	...
	function show_uploaded_file_form(){
		...
		$data = $this->Approval_upload_wa_blast_model->show_activity_by_file($id);
		$header = "<tr><td><input type='checkbox' id='check_all' name='check_all' onchange='allCheck()' ></td><td>No</td><td>Account Number</td><td>Phone Number</td><td>Status Upload</td><td>Message</td><td>Status Approval</td></tr>";
		$detail = "";
		...
		foreach ($data as $key => $value) {
			if ($value['status']=='success') {
                ...
                $html='<td><input type="checkbox" class="checkFalse" id="checkModal_'.$key.'" name="checkModal[]" value="'.$value['id'].'" disabled ></td>';
            }
			$detail .= "<tr>".$html."<td>".($key+1)."</td><td>".$value['account_number']."</td><td>".$value['phone_number']."</td><td>".$value['status']."</td><td>".$value['message']."</td><td>".$value['status_description']."</td></tr>";
		}
		
		$data["upload_id"] = $id;
		$data["table"] = '<table class="table table-striped table-bordered table-hover">'.$header.$detail."</table>";
		// dd($data);
		return view('\App\Modules\ApprovalUploadWaBlast\Views\Show_uploaded_file_form_view', $data);
	}
	function approve_upload_file(){
		...
		$return = $this->Approval_upload_wa_blast_model->approve_upload_file($data);
		if($return){
			$cache = session()->get('USER_ID').'_wa_blast_appove';
	...
	function reject_upload_file(){
		...
		$return = $this->Approval_upload_wa_blast_model->reject_upload_file($data);
		if($return){
			$cache = session()->get('USER_ID').'_wa_blast_appove';
			$this->cache->delete($cache);
			$rs = array('success' => true, 'message' => 'Success to upload data', 'data' => null);
