jQuery(function($) {
	...
	//list sms
	var grid_selector = "#sms-info-list-grid-table";
	var pager_selector = "#sms-info-list-grid-pager";
	
	var ci_controller = GLOBAL_MAIN_VARS["SITE_URL"] + "settings/sms_info_for_aav_list";
	...
	//Add header
	$("#t_sms-info-list-grid-table").css("height", "40px").css("padding", "5px").css("background", "#e7e7e7");
	$("#t_sms-info-list-grid-table").append(
		"<button class='btn btn-sm btn-success' id='btn-sms-add'>Add</button>&nbsp;" +
		"<button class='btn btn-sm btn-primary' id='btn-sms-edit'>Edit</button>&nbsp;" +
		"<button class='btn btn-sm btn-danger' id='btn-sms-delete'>Delete</button>"
	);
	...
	$("#btn-sms-edit").click(function() {
		...
		if(selr)
		{
				...
				"success" :
				 {
					...
					"callback": function() {
						...
						var options = {
							url : GLOBAL_MAIN_VARS["SITE_URL"] + "settings/save_sms_edit/",
							type: "post",
							beforeSubmit: jqformValidate,
							success: showFormResponse,
							dataType: 'json',
			...
			showCommonDialog(900, 500, 'Edit SMS Template', GLOBAL_MAIN_VARS["SITE_URL"] + 'settings/sms_edit_form/' + selr, buttons);	
		} else {
	...
	$("#btn-sms-delete").click(function() {
		...
		if(selr)
		{
			bootbox.confirm("Are you sure you want to delete this SMS content?", function(result) {
				if(result) {
					$.post(GLOBAL_MAIN_VARS["SITE_URL"] + "settings/delete_sms/", { id: selr }, function(responseText) {
						if(responseText.success) {
							showInfo("Berhasil.");
							$(grid_selector).trigger('reloadGrid');
						} else{
