...
jQuery(function($) {
	...
	var ci_controller = GLOBAL_MAIN_VARS["SITE_URL"] + "settings/zipcode_area_mapping_list_temp";
	...
	jQuery(grid_selector).jqGrid({
		...
		height: 340,
		width: null,
		colNames:['ID','SUB AREA ID','SUB AREA NAME','AREA TAGIH', 'PRODUCT','ZIPCODE','TGL PENGAJUAN','STATUS PENGAJUAN','JENIS PENGAJUAN'],
		colModel:[
	...
	$("#t_zipcode_area_mapping-list-grid-table").append(
		...
		"<button class='btn btn-sm btn-primary' id='btn-zipcode_area_mapping-edit'>APPROVE</button>&nbsp;" +
		"<button class='btn btn-sm btn-danger' id='btn-zipcode_area_mapping-delete'>REJECT</button>"
	);
	...
	$("#btn-zipcode_area_mapping-edit").click(function() {
		...
		if(selr)
		{
			bootbox.confirm("Apakah anda yakin akan melakukan approval?", function(result) {
				if(result) {
					$.post(GLOBAL_MAIN_VARS["SITE_URL"] + "settings/save_zipcode_area_mapping_edit_temp/", { id: arr_id, arr_kk : arr_kk }, function(responseText) {
						if(responseText.success) {
							showInfo("Berhasil.");
							$(grid_selector).trigger('reloadGrid');
						} else{
		...
		if(selr)
		{
			bootbox.confirm("Apakah anda yakin akan melakukan reject?", function(result) {
				if(result) {
					$.post(GLOBAL_MAIN_VARS["SITE_URL"] + "settings/save_note_reject_zipcode_area_mapping/", {arr_id: arr_id}, function(responseText) {
						if(responseText.success) {
							showInfo("Berhasil.");
							$(grid_selector).trigger('reloadGrid');
						} else{
	...
	//txt-zipcode_area_mapping_office_id, txt-zipcode_area_mapping_office_name, opt_zipcode_area_mapping
	var jqformValidate = function(jqForm)
{	
	var passed = true;
	if(($("#txt-zipcode_area_mapping-id").val()=="") || ($("#txt-zipcode_area_mapping-name").val()=="")){
		passed = false;
	}
	if(!passed) {
