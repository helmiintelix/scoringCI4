...
jQuery(function($) {
	...
	var ci_controller = GLOBAL_MAIN_VARS["SITE_URL"] + "settings/master_regional_list";
	...
	jQuery(grid_selector).jqGrid({
		...
		height: 340,
		width: null,
		colNames:['ID REGIONAL','NAME REGIONAL', 'ACTUAL NAME','TGL INPUT','TGL UPDATE','STATUS AKTIF','STATUS APPROVAL'],
		colModel:[
			{name:'id',index:'a.id', width:120},
			{name:'name', index:'a.name', width:180},
			{name:'real_name', index:'a.real_name', width:200, cellattr: function (rowId, tv, rawObject, cm, rdata) { 
...
jQuery(grid_selector).jqGrid('filterToolbar',{defaultSearch:true,stringResult:true});		
	...
	$("#t_mr-list-grid-table").append(
		...
		+"<button class='btn btn-sm btn-primary' id='btn-mr-edit'>EDIT</button>&nbsp;"
		+(GLOBAL_SESSION_VARS["LEVEL_GROUP"]!='ROOT'? "":"<button class='btn btn-sm btn-danger' id='btn-mr-delete'>DELETE</button>")
	);

	var grid_app_selector = "#mr-appr-list-grid-table";
	var pager_app_selector = "#mr-appr-list-grid-pager";
	var ci_app_controller = GLOBAL_MAIN_VARS["SITE_URL"] + "settings/master_regional_list_temp";
	...
	$("#btn-mr-add").click(function() {
			...
			"success" :
			 {
				...
				"callback": function() {
					if(($("#txt-id").val()=="")||($("#txt-real_name").val()=="")){
						showWarning("Silakan mengisi field mandatory");
						return false;
					}					
					var options = {
						url : GLOBAL_MAIN_VARS["SITE_URL"] + "settings/save_master_regional_add/",
						type: "post",
		...
		showCommonDialog(900, 500, 'ADD MASTER REGIONAL', GLOBAL_MAIN_VARS["SITE_URL"] + 'settings/master_regional_add_form/', buttons);	
	});
		...
		if(selr)
		{
				...
				"success" :
				 {
					...
					"callback": function() {
						...
						var options = {
							url : GLOBAL_MAIN_VARS["SITE_URL"] + "settings/save_master_regional_edit/",
							type: "post",
							beforeSubmit: jqformValidate,
							success: showFormResponse,
							dataType: 'json',
			...
			showCommonDialog(900, 500, 'EDIT MASTER REGIONAL', GLOBAL_MAIN_VARS["SITE_URL"] + 'settings/masterRegionalEditForm/' + selr, buttons);	
		} else {
	...
	$("#btn-mr-delete").click(function() {
		...
		if(selr)
		{
			bootbox.confirm("Apakah anda yakin akan menghapus?", function(result) {
				if(result) {
					$.post(GLOBAL_MAIN_VARS["SITE_URL"] + "settings/delete_master_regional/", { id: arr_id }, function(responseText) {
						if(responseText.success) {
							showInfo("Berhasil.");
							$(grid_selector).trigger('reloadGrid');
							$(grid_app_selector).trigger('reloadGrid');
