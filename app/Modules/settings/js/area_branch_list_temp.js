...
jQuery(function($) {
	...
	var ci_controller = GLOBAL_MAIN_VARS["SITE_URL"] + "settings/area_branch_list_temp";
	...
	jQuery(grid_selector).jqGrid({
		...
		height: 340,
		width: null,
		colNames:['ID','AREA ID','NAMA AREA', 'PROVINSI AREA','KOTA AREA','KECAMATAN AREA','KELURAHAN AREA','BRANCH LIST','ALAMAT AREA','NO.TELP AREA','MANAGER AREA','TGL PENGAJUAN','STATUS PENGAJUAN','JENIS PENGAJUAN'],
		colModel:[
	...
	$("#t_area_branch-list-grid-table").append(
		...
		"<button class='btn btn-sm btn-primary' id='btn-area_branch-edit'>APPROVE</button>&nbsp;" +
		"<button class='btn btn-sm btn-danger' id='btn-area_branch-delete'>REJECT</button>"
	);
...
/*		var buttons = {
		...
		showCommonDialog(900, 500, 'ADD area_branch GROUP', GLOBAL_MAIN_VARS["SITE_URL"] + 'settings/area_branch_add_form/', buttons);	
*/
	...
	$("#btn-area_branch-edit").click(function() {
		...
		if(selr)
		{
			bootbox.confirm("Apakah anda yakin akan melakukan approval?", function(result) {
				if(result) {
					$.post(GLOBAL_MAIN_VARS["SITE_URL"] + "settings/save_area_branch_edit_temp/", { id: arr_id, arr_kk : arr_kk }, function(responseText) {
						if(responseText.success) {
							showInfo("Berhasil.");
							$(grid_selector).trigger('reloadGrid');
						} else{
		...
		$("#btn-area_branch_office-delete").click(function() {
		var selr = $(grid_selector).jqGrid('getGridParam', 'selrow');
		...
		if(selr)
		{
			bootbox.confirm("Apakah anda yakin akan melakukan reject?", function(result) {
				if(result) {
					$.post(GLOBAL_MAIN_VARS["SITE_URL"] + "settings/delete_area_branch_office/", { id: arr_id, arr_kk: arr_kk }, function(responseText) {
						if(responseText.success) {
							showInfo("Berhasil.");
							$(grid_selector).trigger('reloadGrid');
						} else{
		...
		if(selr)
		{
				...
				"success" :
				 {
					...
					"callback": function() {
						...
						var options = {
							url : GLOBAL_MAIN_VARS["SITE_URL"] + "settings/save_note_reject_area_branch/",
							type: "post",
							beforeSubmit: jqformValidate,
							success: showFormResponse,
							dataType: 'json',
			...
			showCommonDialog(900, 500, 'EDIT area_branch TEMPLATE', GLOBAL_MAIN_VARS["SITE_URL"] + 'settings/note_reject_area_branch/' + selr, buttons);	
			*/

			bootbox.confirm("Apakah anda yakin akan melakukan reject?", function(result) {
				if(result) {
					$.post(GLOBAL_MAIN_VARS["SITE_URL"] + "settings/save_note_reject_area_branch/", {id: arr_id}, function(responseText) {
						if(responseText.success) {
							showInfo("Berhasil.");
							$(grid_selector).trigger('reloadGrid');
						} else{
...
/*	
	$("#btn-area_branch-delete").click(function() {
		...
		if(selr)
		{
			bootbox.confirm("Are you sure you want to delete this area_branch content?", function(result) {
				if(result) {
					$.post(GLOBAL_MAIN_VARS["SITE_URL"] + "settings/delete_area_branch/", { id: selr }, function(responseText) {
						if(responseText.success) {
							showInfo("Berhasil.");
							$(grid_selector).trigger('reloadGrid');
						} else{
...
*/
	...
	//txt-area_branch_office_id, txt-area_branch_office_name, opt_area_branch
	var jqformValidate = function(jqForm)
{	
	var passed = true;
	if(($("#txt-area_branch-id").val()=="") || ($("#txt-area_branch-name").val()=="")){
		passed = false;
	}
	if(!passed) {
