...
jQuery(function($) {
	...
	var ci_controller = GLOBAL_MAIN_VARS["SITE_URL"] + "settings/area_list_temp";
	...
	jQuery(grid_selector).jqGrid({
		...
		height: 340,
		width: null,
		colNames:['ZIP REC','ACTION','KODEPOS','PROVINSI','KABUPATEN','KECAMATAN','KELURAHAN','ID','TGL INPUT','TGL UPDATE','STATUS AKTIF','STATUS APPROVAL'],
		colModel:[
			...
			{name:'id', index:'a.id', width:200, hidden:true},
...
*/
			bootbox.confirm("Apakah anda yakin akan melakukan approval?", function(result) {
				if(result) {
					$.post(GLOBAL_MAIN_VARS["SITE_URL"] + "settings/edit_flag_chekermaker/", { id: arr_id, kel: arr_kel,arr_k: arr_k }, function(responseText) {
						if(responseText.success) {
							showInfo("Berhasil.");
							$(grid_selector).trigger('reloadGrid');
						} else{
	...
	$("#btn-area-delete").click(function() {
		...
		if(selr)
		{
			bootbox.confirm("Apakah anda yakin akan melakukan reject?", function(result) {
				if(result) {
					$.post(GLOBAL_MAIN_VARS["SITE_URL"] + "settings/delete_area/", { id: arr_id, kel: arr_kel, k: arr_k  }, function(responseText) {
						if(responseText.success) {
...
{	
	var passed = true;
	if(($("#txt-zip_reg").val()=="") || ($("#txt-kelurahan").val()=="") || ($("#txt-kodepos").val()=="")){
		passed = false;
	}
	if(!passed) {
