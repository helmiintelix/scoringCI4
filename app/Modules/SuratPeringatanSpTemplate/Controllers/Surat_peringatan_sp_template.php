...
use App\Modules\SuratPeringatanSpTemplate\Models\Surat_peringatan_sp_template_model;


class Surat_peringatan_sp_template extends \App\Controllers\BaseController
{

	function __construct()
	{
		$this->Surat_peringatan_sp_template_model = new Surat_peringatan_sp_template_model();
	}

	function letter_template()
	{
		
		$data['classification'] = $this->input->getPost('classification');
		
		return view('\App\Modules\SuratPeringatanSpTemplate\Views\Surat_peringatan_sp_template_view',$data);
	}
	...
	function letter_list()
	{
		...
		$cache = session()->get('USER_ID').'_surat_peringatan_sp_template';

		if($this->cache->get($cache)){
			$data = json_decode($this->cache->get($cache));
			$rs = array('success' => true, 'message' => '', 'data' => $data);
		}else{
			$data = $this->Surat_peringatan_sp_template_model->get_letter_list();
	...
	function letter_list_temp(){
		$cache = session()->get('USER_ID').'_surat_peringatan_sp_template_temp';

        if($this->cache->get($cache)){
			$data = json_decode($this->cache->get($cache));
			$rs = array('success' => true, 'message' => '', 'data' => $data);
		}else{
			$data = $this->Surat_peringatan_sp_template_model->get_letter_list_temp();
			$this->cache->save($cache, json_encode($data), env('TIMECACHE_1')); 
	...
	// 	return view('\App\Modules\SuratPeringatanSpTemplate\Views\setup_area_branch_add_form_view',$data);
	// }
	...
	// 		echo "NOT FOUND";
	// 	}else{
	// 		return view('\App\Modules\SuratPeringatanSpTemplate\Views\setup_area_branch_edit_form_view',$data);
	// 	}
	...
	// 	$is_exist = $this->Surat_peringatan_sp_template_model->isExist($this->input->getPost('txt-branch-id'));
	...
	// 		$return	= $this->Surat_peringatan_sp_template_model->save_area_branch_add($data);
	...
	// 	$is_exist = $this->Surat_peringatan_sp_template_model->isExistarea_branch_tempId($this->input->getPost('txt-id'));
	...
	// 		$return	= $this->Surat_peringatan_sp_template_model->save_area_branch_edit($area_branch_data);
