...
var loadMenu = function (title, link, menu_id) {
  ...
  try {
    $("#admin-wrapper")
      .html(GLOBAL_MAIN_VARS["SPINNER"])
      .load(
        GLOBAL_MAIN_VARS["BASE_URL"] + "/" + link,
  ...
  $.each(menu_arrx, (i, val) => {
    ...
    if (val["children"]) {
      ...
      $.each(val["children"], (ii, vall) => {
        ...
        // }
        html +=
          '<i class="bi bi-list-ul"></i><span>' + vall["menu_desc"] + "</span>";

        if (vall["children"]) {
          html +=
            '<i class="bi bi-chevron-down ms-auto" style="font-size: 8px;"></i>';
        }
        html += " </a>";
        if (vall["children"]) {
          html +=
            ' <ul id="' +
            href_sub +
            '" class="nav-content collapse" style="margin-left: 0px;">';
...
jQuery(function ($) {
  ...
  //Main Screen Init
  if (
    GLOBAL_SESSION_VARS["LEVEL_GROUP"] == "ADMIN" ||
    GLOBAL_SESSION_VARS["LEVEL_GROUP"] == "ROOT"
  ) {
...
function loadWAModule() {
  $("#wa2wayCanvas")
    .html("")
    .load(
      GLOBAL_MAIN_VARS["BASE_URL"] + "whatsappConversation/wa2wayListView",
      function (responseTxt, statusTxt, xhr) {
        if (statusTxt == "success") {
          changeTheme(GLOBAL_THEME_MODE);
          $("#wa2wayCanvas").show();
