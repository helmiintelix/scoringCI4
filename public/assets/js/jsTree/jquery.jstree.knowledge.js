...
(function ($) {
	$.jstree.plugin("ui", {
		__init : function () { 
			...
			this.get_container()
				//when tree is clicked - Daniel
				.delegate("a", "click.jstree", $.proxy(function (event) {
					event.preventDefault();
					this.select_node(event.currentTarget, true, event);
					var file_path = event.currentTarget;
					var target = event.target || event.srcElement;
					
					$.cookie('id_tree', this.data.ui.to_select);
					$.cookie('file_path', file_path);					
					
					$("#txt_start_date").val(target.id);
					$("#txt_end_date").val(target.name);
					$("#knowledge_content").attr('src', file_path);
				}, this))
				...
				if($.isArray(js)) {
					...
					$.each(js.data, function (i, m) {
						...
						if(typeof m == "string") { 
							...
							//edit untuk file_path, start_date and end_date - Daniel
							if(js.file_path == "") {
								tmp.attr('href','#')[ t ? "html" : "text" ](m);
							} else {
								tmp.attr('href',js.file_path)[ t ? "html" : "text" ](m); 						
								tmp.attr('target','knowledge_content')[ t ? "html" : "text" ](m); 
								tmp.attr('id',js.start_date)[ t ? "html" : "text" ](m);
								tmp.attr('name',js.end_date)[ t ? "html" : "text" ](m);
							}
						}
						else {
							if(!m.attr) { m.attr = {}; }
