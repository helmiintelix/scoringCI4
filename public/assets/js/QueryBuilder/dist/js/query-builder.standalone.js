                    ...
                    for (var j = 0; j < tempValue.length; j++) {
                        switch (QueryBuilder.types[filter.type]) {
                            ...
                            case 'number':
                                ...
                                if(tempValue[j].match(/(?=.*?\d)^\$?(([1-9]\d{0,2}(,\d{3})*)|\d+)?(\.\d{1,2})?$/)){	
                                    result = ['number_nan'];
                                    break;
                                }
                                */ // sudah dihandle ketika input
                                //sudah dihandle input
                                /*
...
<select class="form-control {{? it.operators.length === 1 }}hide{{?}}" name="{{= it.rule.id }}_operator"> \
  ...
  {{? optgroup !== null }}</optgroup>{{?}} \
    ...
    /* Tri : 20190531 Function specific for Collection */
    /*function QueryBuilderPushValue(search_url,name){
        ...
        showCommonDialog3(900, 500, 'Pilih Parameter',search_url, buttons);	                  
    ...
    QueryBuilder.prototype.getRuleInput = function (rule, value_id) {
        ...
        if (typeof filter.input == 'function') {
            ...
            switch (filter.input) {
                ...
                case 'number':
                    //Tri : 20190715 coba tambah separator
                    /*
                    ...
                    if (filter.size) h += ' size="' + filter.size + '"';
                    h += '>';
                    h += '<script type="text/javascript">$("[name=' + name + ']").maskNumber({integer: true, thousands: ","});  </script>';
