...
socketPushNotif.on("connect", () => {
    //console.log('socket');
    socketPushNotif.emit("register", { USER_ID: USER_IDX, SOCKET_ID: socketPushNotif.id }, (response) => {
...
var sendNotification = (notification_id) => {
    socketPushNotif.emit("pushnotif",
        {
...
var renderNotif = (data) => {
    ...
    if (isRead == '0') {
        ...
        notif = '<span id="flagNew_' + data.notification_id + '" class="badge bg-danger float-sm-end" style="font-size: 8px;">New</span>';
    }



    let html = '<li id="notification_' + data.notification_id + '" class="notification-item" ' + style + ' onclick="loadMenu(\'' + data.menu_desc + '\' ,\'' + data.url + '\');readNotif(\'' + data.notification_id + '\')" > ' +
        icon +
        '<div style="width:100%"> ' +
        '<div class="row" style="margin-bottom: -6px;">' +
        '<div class="col">' +
        '<p class="text-muted" style="font-style: italic;opacity: 0.5;font-size:11px">from: ' + data.fromName + '</p>' +
        '</div>' +
        '<div class="col">' +
        notif +
        '</div>' +
...
var timeDif = (time) => {
    ...
    if (bulanTahunNotif == bulanTahunCurrent) { //jika di bulan dan tahun yang sama
        if (date1.getDate() == date2.getDate()) { //jika di hari yang sama
            ...
            else if (hh == 1) { //selisih 1 jam
                return "an hour ago";
            } else if (hh < 1 && mm > 2) { //selisih kurang dari 1 jam dan lebih dari 2 menit
                return mm + " minutes ago";
            } else {
                return "just now";
            }
        } else if (date1.getDate() != date2.getDate()) { //jika di hari yang berbeda
            ...
            if (selisih == 1) {
                ...
                let tanggal = date1.getDate() < 10 ? '0' + date1.getDate() : date1.getDate();
                let bulan = (date1.getMonth() + 1) < 10 ? '0' + date1.getMonth() : date1.getMonth();
