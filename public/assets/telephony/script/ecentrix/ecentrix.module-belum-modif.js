...
// load
var loadTelephonyModule = function (server, port, policy, agent, ip, hosted, sip, rtmp, level){
	...
	if (FlashDetect.installed && FlashDetect.versionAtLeast(10)){
		...
		swfobject.embedSWF(
				"../public/telephony/swf/agentdesktop-2.0.3.swf", 
				"controller", 
				"1", "1", 
				"10.0.0", 
...
var clientAppIsLoaded = function(){
	...
	setTimeout(function(){
		$("#loader").remove();
		$("#telephony .wrapper").css("display", "block");
		TELEPHONY_CONTROLLER.login();
	}, 1000);
	// register
	$(window).unload(function(){
		if (TELEPHONY_CONTROLLER!=null){
...
var clientAppProgress = function(percentage){
	var progress = 90 + (parseInt(percentage, 10) / 10);
	$("#bar").width(progress + "%");
};
	...
	try {
		...
		if (result=="SUCCESS"){
			...
			if (TELEPHONY_CURRENT_STATUS=="LOGIN" || TELEPHONY_CURRENT_STATUS=="ON_HOOK"){
				...
				if (TELEPHONY_CURRENT_STATUS=="ON_HOOK"){
					$("#agent_status").css("color", "#FF0000").text("DISCONNECT");
				}
			}
		...
		if (status=="LOGIN"){
			if (reason!==undefined && reason.length>0 && !isNaN(reason)){
				...
				TELEPHONY_EXTENSION_NUMBER = reason;
				$("#extension_no").text(TELEPHONY_EXTENSION_NUMBER); //by Martz
				
				//Save Extension
				$.ajax({
					type: "POST",
					url: site_url+"/agent_main/save_extension/",
					async: false,
					dataType: "json",
					data: { ext_no: TELEPHONY_EXTENSION_NUMBER },
					success: function(msg){
					 	//
					},
					error: function(){
						//warningDialog(300,100,'Warning !', "Loading costumer data failed!");
					}
				});
			}
		}
 ...
 */
var callAlert = function (callerId, number, customerData, recordingId){
	TELEPHONY_LOG.info("callAlert [callerId: "+ callerId +", number: "+ number + ", customerData: "+ customerData + ", recordingId: "+ recordingId + "]");		
	callActiveState();
	...
	try {
		 ...
		 * do something here
		 */
		predialAction(callerId, number, customerData, recordingId);
	}
	catch (ex){
		TELEPHONY_LOG.error(
				"Error while running 'callAlert' function with param [callerId: "+ callerId +", number: "+ number + ", customerData: "+ customerData + ", recordingId: "+ recordingId + "] because of exception",
				ex
			);
	}
};
...
var callOriginated = function (callerId, number){
	...
	try {
		TELEPHONY_CALLER_ID = callerId;
		GLOBAL_MAIN_VARS['TELEPHONY_CALLER_ID'] = callerId; //added by Martz @ 20111108
		//alert(GLOBAL_MAIN_VARS['TELEPHONY_CALLER_ID']);
		// update status
 ...
 * @param {String} recordingId
 */
var callConnected = function (callerId, number, recordingId){
	callActiveState();
	TELEPHONY_LOG.info("callConnected [callerId: "+ callerId +", number: "+ number +", recordingId: "+ recordingId +"]");
	try {
		TELEPHONY_CALLER_ID = callerId;
		...
		/*Connect Time*/
		var now = new Date();
		Data.ConnectTimeLog = now.getFullYear() + "-" + (now.getMonth() + 1) + "-" + now.getDate() + " " + now.getHours() + ":" + now.getMinutes() + ":" + now.getSeconds();
		
	}
	catch (ex){
		TELEPHONY_LOG.error(
				"Error while running 'callConnected' function with param [callerId: "+ callerId +", number: "+ number + ", recordingId: "+ recordingId +"] because of exception",
				ex
			);
	}
};
...
var callDisconnected = function (callerId, number){
	...
	try {
		...
		$("#telephony input#a_number").val("< type a number in here >");
		...
		/*Connect Time*/
		var now = new Date();
		Data.DisconnectTimeLog = now.getFullYear() + "-" + (now.getMonth() + 1) + "-" + now.getDate() + " " + now.getHours() + ":" + now.getMinutes() + ":" + now.getSeconds();
	}
	catch (ex){
		TELEPHONY_LOG.error(
...
var callJoined = function (callerId, number, conferenceId){
	...
	try {
		...
		if ($("#telephony button.join").length==0){
			...
			// wrap with corner
			$("#telephony button.join[data="+ conferenceId +"]").show().wrap("<div class='outer'></div>");
			if (!$.browser.msie){
			...
			// wrap with corner
			$("#telephony button.leave[data="+ conferenceId +"]").show().wrap("<div class='outer'></div>");
			if (!$.browser.msie){
		...
		if ($("#telephony button.kick").length==0){
			...
			// wrap with corner
			$("#telephony button.kick[data="+ conferenceId +"]").show().wrap("<div class='outer'></div>");
			if (!$.browser.msie){
...
var originateCall = function (number, customerData, timeout, dialOptions, originator){
	number = $.trim(number);
	$("#agent_status").css("color", "#008000");
	if (!isNaN(number)&& number.length>0 && number !== "< type a number in here >"){
...
var parkCall = function (){
	if (TELEPHONY_CALLER_ID!=null){
		if ($("#telephony button.unpark[data="+ TELEPHONY_CALLER_ID +"]").length<=0){
			...
			// wrap with corner
			$("#telephony button.unpark[data="+ TELEPHONY_CALLER_ID +"]").show().wrap("<div class='outer'></div>");
			if (!$.browser.msie){
		...
		if (TELEPHONY_CALLER_ID!=null){
			try {
				...
				if (timeLimit!==undefined){
					...
					getController().callJoinConference(TELEPHONY_CALLER_ID, TELEPHONY_CONFERENCE_ID);
				}
			}
			catch (ex){
...
var createTelephonyMenuHandler = function (id, textButton){
	var hide = function(){
		//$("#telephony #"+ id +".menu").hide("slide", {direction: "down"}, 100);
		$("#telephony #"+ id +".menu").hide();
	};
	var show = function(){
		if ($.browser.msie){
			$("#telephony #"+ id +".menu").show();
		}
		else {
			$("#telephony #"+ id +".menu").show("slide", {direction: "down"}, 100);
		}
	};
	...
	$("#telephony button:contains('"+ textButton +"')").hover(
		function(e){
			if (!isShown()){
				show();
			}
...
$(document).ready(function(){
	/*by Martz - time worked*/
	//if(jQuery.isFunction(jQuery.timeago)){
		//$("#dt").text(jQuery.timeago($("#login_datetime").val()));
		//$("#dt").text($("#login_datetime").val());
  //} 
	...
	$("#transfer_dialog").dialog({
		...
		close: function() {
			$(this).find("input[name='agent_target']").val("");
		}
	});
	$("input[name='agent_target']").keypress(function (e){
		if (e.which==13){
			transferCall();
	...
	*/
	...
	// TODO: test only, remove if done
	//TELEPHONY_LOG.info($("#telephony .wrapper:eq(2)").html());
	/*
	var height = $("#telephony").height() + "px";
	var offset = $("#telephony .wrapper:eq(2)").offset();
	var leftPos = offset.left + "px";
	$("#telephony .wrapper:eq(2)").css({
		"position": "fixed",
		"top": "auto",
		"bottom": height,
		"left": leftPos
