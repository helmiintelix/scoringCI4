...
var loadTelephonyModule = function (server, port, policy, agent, ip, hosted, sip, rtmp, level){
	...
	if (FlashDetect.installed && FlashDetect.versionAtLeast(10)){
		...
		swfobject.embedSWF(
				"../public/telephony/swf/agentdesktop-2.0.1.swf", 
				"controller", 
				"1", "1", 
				"10.0.0", 
				"../public/telephony/swf/expressInstall.swf", 
				moduleFlashVars, 
				moduleParams, 
				moduleAttributes, 
				function (e){
...
var flashDebug = function(message){
	//showConnectionStatus("[DEBUG] "  + message);
};
var flashInfo = function(message){
	//showConnectionStatus("[INFO ] "  + message);
};
var flashWarn = function(message){
	//showConnectionStatus("[WARN ] "  + message);
};
 ...
 */
var originateCall = function (number, customerData, dialOptions, originator){
	number = $.trim(number);
	if (!isNaN(number)&& number.length>0 && number !== "< type a number in here >"){
		...
		*/
		TELEPHONY_LOG.debug("originateCall [number: "+ number +", customerData: "+ (customerData!==undefined?customerData:"") +", dialOptions: "+ (dialOptions!==undefined?dialOptions:"") +", originator: "+ (originator!==undefined?originator:"") + "]");
		try {
			if (number!==undefined){
				// reset status
					...
					if (customerData!==undefined){
						if (dialOptions!==undefined){
							if (originator!==undefined){
								getController().originateCall(number, customerData, dialOptions, originator);
							}
							else{
								getController().originateCall(number, customerData, dialOptions);
							}
						}
						else {
							getController().originateCall(number, customerData);
						}
					}
					else {
						getController().originateCall(number);
					}
				}
			}
		}
...
var resetActionMenuPosition = function (){
	var actionButtonOffset = $("#telephony button:contains('action')").offset();
	var left = actionButtonOffset.left-3;
