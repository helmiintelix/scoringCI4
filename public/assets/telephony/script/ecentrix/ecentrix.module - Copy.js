...
// load flash
var loadFlash = function (server, port, policy, agent, ip, hosted){
	if (FlashDetect.installed && FlashDetect.versionAtLeast(10)){
		...
		swfobject.embedSWF(
			"../public/telephony/swf/agentdesktop-2.0.8.swf", 
			"controller", 
			"1", "1", 
			"10.0.0", 
...
// load applet
var loadApplet = function (server, port, policy, agent, ip, hosted){
	if (navigator.javaEnabled() && deployJava.versionCheck("1.7.0*")){
		var attributes = {
				name: "eCentriX",
				id: "eCentriX",
				codebase: "jar",
				code: "AgentDesktopController.class",
				archive: "desktop-2.0.2.jar",
				"class": "applet",
				mayscript: true,
				scriptable: true,
				width: 0,
...
// load
var loadTelephonyModule = function (connector, server, port, policy, agent, ip, hosted, sip, rtmp, level){
	...
	if (TELEPHONY_CONNECTOR==="applet"){
		// load applet
		//loadApplet(server, port, policy, agent, ip, hosted);
	}
	else if (TELEPHONY_CONNECTOR==="flash"){
		// load flash
		loadFlash(server, port, policy, agent, ip, hosted);
	}
};
// ------------------------------------------------------------------------
var checkIfLoginAvailable = function(){
	...
	if (TELEPHONY_CONTROLLER != null && TELEPHONY_CONTROLLER !== undefined){
		//if (($.browser.msie && TELEPHONY_CONNECTOR == "applet") || typeof TELEPHONY_CONTROLLER["login"] == "function"){
		if (typeof TELEPHONY_CONTROLLER["login"] == "function"){
		$("#loader").remove();
...
var callJoined = function (callerId, number, conferenceId){
	...
	try {
		...
		if ($("#telephony button.join").length==0){
			...
			$("#telephony span#call_status").html("CONFERENCE");
		}
 ...
 */
var auto_conference = function (extension){
	/*
	createRoom();
	var conferenceId = function (value){
		TELEPHONY_CONFERENCE_ID = value;
	...
	var callJoined = function (callerId, number, conferenceId){ 
		...
		if (number != "4237"){
			...
			setTimeout(function(){
				agentJoinConference(TELEPHONY_CONFERENCE_ID);
			}, 500);
		}
	}
	var callConnected = function (callerId, number, recordingId, recordingFile){
...
/*
	...
	//Put Customer in room
	setTimeout(function(){
		callJoinConference(TELEPHONY_CONFERENCE_ID);
		...
		setTimeout(function(){
			...
			//Put Supervisor in room
			setTimeout(function(){
				callJoinConference(TELEPHONY_CONFERENCE_ID);
				
				//Agent enter the room
				setTimeout(function(){
					outbound();
					alert(TELEPHONY_CONFERENCE_ID + "test");
					agentJoinConference(TELEPHONY_CONFERENCE_ID);
				}, 5000);
	...
	*/
	...
	//alert("createRoom() - next 2s");
	createRoom();
	
	//Put Customer in room
	setTimeout(function(){
		//alert("callJoinConference() - next 5s");
		callJoinConference(TELEPHONY_CONFERENCE_ID);
		
		//Outbound
		setTimeout(function(){
			//alert("outbound() - next 2s");
			outbound();
			
			//Call Supervisor
			setTimeout(function(){
				//alert("originateCall('4237') - next 2s");
				originateCall(extension);
				...
				setTimeout(function(){
					...
					setTimeout(function(){
						...
						setTimeout(function(){
							//alert("callJoinConference() - next end");
							agentJoinConference(TELEPHONY_CONFERENCE_ID);
						}, 1000);
					}, 2000);
				}, 5000);
