    ...
    QueryBuilder.templates.group = ({ group_id, level, conditions, icons, settings, translate, builder }) => {
        return `
                <div id="${group_id}" class="rules-group-container">
                    <div class="rules-group-header" style="display:flex; justify-content:space-between;">
        ...
        if (typeof filter.input == 'function') {
            ...
            switch (filter.input) {
                ...
                default:
                    ...
                    h += '>';
                    if (filter.search) {
                        var hdum = '<button type="button" class="btn btn-xs btn-info" onclick="QueryBuilderPushValue(\'' + filter.search_url + '\',\'' + name + '\')	"> \
                    <i class="icon-search nav-search-icon"></i> Search \
                  </button> ';
                        h += hdum;
...
<div' + style + ' class="' + filter.input + ' ' + filter.input + '-' + color + '"> \
  ...
  <label for="' + id + '">' + val + '</label> \
     ...
     * @param {object} [options]
     * @param {string} [options.icon='bi bi-shuffle']
    ...
    QueryBuilder.define('invert', function (options) {
        ...
        if (!options.disable_template) {
            this.on('getGroupTemplate.filter', function (h) {
                ...
                var $h = $($.parseHTML(h.value));
                $h.find(Selectors.condition_container).append(
                    '<button type="button" class="invert btn btn-xs btn-info" data-invert="group">' +
            ...
            if (options.display_rules_button && options.invert_rules) {
                this.on('getRuleTemplate.filter', function (h) {
                    ...
                    $h.find(Selectors.rule_actions).prepend(
                        '<button type="button" class="invert btn btn-xs btn-info" data-invert="rule">' +
                        '<i class="' + options.icon + '"></i> ' + self.translate('invert') +
                        '</button>'
                    );
                    h.value = $h.prop('outerHTML');
    ...
    QueryBuilder.selectors.invert = '.invert';
     ...
     * @param {object} [options]
     * @param {string} [options.icon_checked='bi bi-toggle-oned']
     * @param {string} [options.icon_unchecked='bi bi-toggle-off']
     */
    QueryBuilder.define('not-group', function (options) {
        ...
        if (!options.disable_template) {
            this.on('getGroupTemplate.filter', function (h) {
                ...
                $h.find(QueryBuilder.selectors.condition_container).prepend(
                    '<button type="button" class="btn btn-xs btn-warning" data-not="group">' +
        ...
        icon_unchecked: 'bi bi-toggle-off',
        icon_checked: 'bi bi-toggle-on',
        disable_template: false
    });
     ...
     * @param {string} [options.icon='bi bi-grip-vertical']
