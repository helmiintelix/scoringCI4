...
function getData() {
  $.ajax({
    url:
      ...
      "account_handling/setup_account_tagging_list/get_collection_list" +
      classification,
    type: "get",
...
var eGridDiv = document.getElementById("myGridSatm");
...
$("#btn-set-account-tagging").click(function () {
  var opt_account_tagging = $("#opt-account-tagging").val();
  if (account_no == "") {
    ...
    $.ajax({
      ...
      url:
        GLOBAL_MAIN_VARS["SITE_URL"] +
        "account_handling/setup_account_tagging_list/set_account_tagging",
      data:
        "account_no=" +
        account_no +
        "&account_tagging=" +
...
$("#btn-del-account-tagging").click(function () {
  bootbox.confirm(
    "Apakah anda yakin akan menghapus Account Tag ?",
    function (result) {
      if (result) {
        if (account_no == "") {
          showWarning("Silakan pilih data");
          ...
          $.ajax({
            ...
            url:
              ...
              "account_handling/setup_account_tagging_list/set_account_tagging",
            data:
              "account_no=" + account_no + "&account_tagging=" + "&mode=add",
            dataType: "json",
            success: showFormResponse,
          });
        }
