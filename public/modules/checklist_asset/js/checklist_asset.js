...
function remove_field(field) {
    ...
    $("#remove-" + field).closest('.form-group').remove();
}
...
function addFieldAsset(assetType) {
    ...
    data = '<div class="form-group">'+
            ...
            '<div class="row">'+
                '<div class="col-lg-5">'+
                    '<input type="text" class="form-control mandatory" name="txtFieldName[' + parseInt(inc) + ']"  id="txtFieldName" placeholder="Field name" data-required="true" required>'+
                '</div>'+
                '<div class="col-lg-1">'+
                    '<input type="text" class="form-control mandatory" name="txtFieldOrder[' + parseInt(inc) + ']" id="txtFieldOrder" placeholder="Sort" required>'+
                '</div>'+
                '<div class="col-lg-1">'+
                    '<div id="addAttr_' + parseInt(inc) + '"></div>'+
                    '<a class="btn btn-danger btn-sm pull-left" href="#" id="remove-' + parseInt(inc) + '" href="#" onClick="remove_field(' + parseInt(inc) + ')"><i class="bi bi-dash-circle"></i></a>' +
                    '</div>'+
            '</div>'+
        '</div>';
	...
	$('#btnAddField-' + $('#opt-asset').val()).click(function () {
        ...
        data = '<div class="form-group">'+
            ...
            '<div class="row">'+
                ...
                '<div class="col-lg-1">'+
                    '<div id="addAttr_' + parseInt(inc) + '"></div>'+
                    '<a class="btn btn-danger btn-sm pull-left" href="#" id="remove_field"><i class="bi bi-dash-circle"></i></a>' +
                    '</div>'+
            '</div>'+
        '</div>';
...
function form_add_submit(form_add) {
    if (checkValidate()) {
        ...
        var options = {
            url: GLOBAL_MAIN_VARS["SITE_URL"] + "checklist_asset/checklist_asset/save_field_checklist",
...
$('#opt-asset').change(function () {
    if (selectedAsset !== '') {
        ...
        console.log('halo')
        $('#formAdd-' + asset_prev).addClass('hide');
        $('#formAdd-' + $(this).val()).addClass('hide');
    }
});
