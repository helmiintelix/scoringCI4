...
function getData() {
  $.ajax({
    url:
      ...
      "visit_radius/visit_radius_temp/visit_radius_list_temp" +
      classification,
    type: "get",
...
// var session = '<?php echo $_SESSION['expire']; ?>';
    ...
    bootbox.confirm(
      ...
      function (result) {
        if (result) {
          $.get(
            GLOBAL_MAIN_VARS["SITE_URL"] +
              "visit_radius/visit_radius_temp/save_visit_radius_approve",
            {
              id: selr,
              // csrf_security: csrfHash,
              // csrf_hash: csrfHash,
            },
            function (data) {
              console.log(data);
  ...
  if (selr) {
    bootbox.confirm("Are you sure to reject this request ?", function (result) {
      if (result) {
        $.get(
          GLOBAL_MAIN_VARS["SITE_URL"] +
            "visit_radius/visit_radius_temp/save_visit_radius_reject",
          {
            id: selr,
            // csrf_security: csrfHash,
            // csrf_hash: csrfHash,
