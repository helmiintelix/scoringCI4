   ...
   $('#loading_checker').show();
    ...
    $.ajax({
			...
			success: function(msg){
                ...
                html += '<select class="form-control form-control-sm" id="checked_by" name="checked_by[]" style="margin-top: 10px;">';
                ...
                html += '<span class="bi bi-x-square" style="color:red;cursor: pointer;margin-top: 5px;margin-left: 10px;color:red;" onClick="delListChecker(this)" data-toggle="tooltip" data-placement="top" title="Delete user"></span>';
                html += '</span>';
                html += '</div>';
...
function addLevelApproval(elm){
    ...
    let arrLevel = $('#arrlevel').val()+'|approval-by-'+lastNumberOfLevel+'[]';
    ...
    $.ajax({
            ...
            success: function(msg){
                ...
                let html = '';
                html += '<div class="form-group" id="form-approval-' + lastNumberOfLevel + '">';
                html += '<div class="col-sm-12">';
                html += '<span onClick="delLevel(' + lastNumberOfLevel + ')" data-toggle="tooltip" data-placement="top" title="Hapus level approval" class="bi bi-dash-circle" style="margin-right: 6px; color: red; cursor: pointer;"></span>';
                html += '<label for="approval-by-' + lastNumberOfLevel + '" style="color: cornflowerblue" class="lbl-approval">Approval ' + lastNumberOfLevel + '</label>';
                html += '<br>';
                html += '<div class="input-group">';
                html += '<select class="form-control form-control-sm" id="approval-by-' + lastNumberOfLevel + '" name="approval-by-' + lastNumberOfLevel + '[]" style="margin-top: 0px;">';
                html += '<option value="">[Select User]</option>';
                $.each(msg.data, function(i, val) {
                    html += '<option value="' + val['value'] + '">' + val['value'] + ' - ' + val['item'] + '</option>';
                });
                ...
                html += '<span class="bi bi-plus-square" style="cursor: pointer; margin-left: 5px;" id="btn-add-approval-' + lastNumberOfLevel + '" onClick="addApproval(this, ' + lastNumberOfLevel + ')" data-toggle="tooltip" data-placement="top" title="Menambahkan approval"></span>';
                html += '<img src="'+GLOBAL_MAIN_VARS["SITE_URL"] +'/assets/img/loading.gif" class="pull-right" id="loading-approval-' + lastNumberOfLevel + '" style="margin-top: 5px; display: none;">';
                html += '</div>';
                html += '</div>'; 
                html += '</div>'; 
                html += '</div>'; 
     ...
     $.ajax({
            type: "get",
             ...
             success: function(msg){
                ...
                html += '<div class="input-group">';
                html += '<select class="form-control form-control-sm" id="approval-by-'+num+'" name="approval-by-'+num+'[]" style="margin-top: 10px;">';
                html += '<option value="">[Select User]</option>';
                $.each(msg.data,function(i,val){
                    html +='<option value="'+val['value']+'">'+val['value']+' - '+val['item']+'</option>';
                });                         
...
function delLevel(elm){
   ...
   console.log(elm);
   let arrLevel = $('#arrlevel').val().replace('|approval-by-'+elm+'[]','');
