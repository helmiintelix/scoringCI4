...
function getData() {
  $.ajax({
    url:
      ...
      "settings/setup_alert_angsuran_approval/get_setup_angsuran_temp" +
      classification,
    type: "get",
    ...
    bootbox.confirm(
      ...
      function (result) {
        if (result) {
          $.get(
            GLOBAL_MAIN_VARS["SITE_URL"] +
              "settings/setup_alert_angsuran_approval/approve_system_setting",
            {
              id: selr,
            },
            function (data) {
  ...
  if (selr) {
    bootbox.confirm("Are you sure to reject this request ?", function (result) {
      if (result) {
        $.get(
          GLOBAL_MAIN_VARS["SITE_URL"] +
            "settings/setup_alert_angsuran_approval/reject_system_setting",
          {
            id: selr,
          },
          function (data) {
