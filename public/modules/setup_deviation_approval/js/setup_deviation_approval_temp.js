...
function flagRenderer(params) {
  const link = params.data.jenis_request;
  return link;
}
...
var gridOptions = {
  ...
  onCellClicked: (params) => {
    ...
    selr = params.data.id;
    deviation_approval_id = params.data.dev_app_id;
    ...
    bootbox.confirm(
      ...
      function (result) {
        if (result) {
          $.get(
            GLOBAL_MAIN_VARS["SITE_URL"] +
              "settings/deviation_approval_temp/save_deviation_approval_edit_temp",
            {
              id: selr,
              deviation_approval_id: deviation_approval_id,
            },
            function (data) {
              console.log(data);
  ...
  if (selr) {
    bootbox.confirm("Are you sure to reject this request ?", function (result) {
      if (result) {
        $.get(
          GLOBAL_MAIN_VARS["SITE_URL"] +
            "settings/deviation_approval_temp/save_note_reject_deviation_approval",
          {
            id: selr,
          },
          function (data) {
