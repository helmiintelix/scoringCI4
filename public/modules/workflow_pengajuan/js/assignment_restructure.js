...
function getData() {
  $.ajax({
    url:
      ...
      "workflow_pengajuan/workflow_pengajuan_restructure/workflow_pengajuan_restructure_list" +
      classification,
    type: "get",
    data: { status: status },
  ...
  onCellDoubleClicked: (params) => {
    ...
    if (params.data.status == "NEW") {
      ...
      showCommonDialog(
        ...
        GLOBAL_MAIN_VARS["SITE_URL"] +
          "workflow_pengajuan/workflow_pengajuan_restructure/pengajuan_program_form?id=" +
          params.data.id +
          "&account_id=" +
...
var eGridDiv = document.getElementById("myGridAr");
...
$("#btn-request-only-flag").click(function () {
      ...
      callback: function () {
        ...
        setTimeout(() => {
          showCommonDialog(
            ...
            "RESTRUCTURE PROGRAM REQUEST",
            GLOBAL_MAIN_VARS["SITE_URL"] +
              "workflow_pengajuan/workflow_pengajuan_restructure/pengajuan_program_form?id=null&account_id=" +
              cm_card_nmbr +
              "&source=CMS&agent_id=" +
              GLOBAL_VARS +
              "&screen_level=NEW",
        ...
        //   url:
        //     GLOBAL_MAIN_VARS["SITE_URL"] +
        //     "/workflow_pengajuan/workflow_pengajuan_restructure/saveNewRequest",
  ...
  showCommonDialog(
    ...
    "SEARCH",
    GLOBAL_MAIN_VARS["SITE_URL"] +
      "workflow_pengajuan/workflow_pengajuan_restructure/requestData",
    buttons
  );
});
  ...
  if (selected_data.status) {
    if (selected_data.status == "NEW") {
        ...
        success: {
          ...
          callback: function () {
            $.ajax({
              ...
              url:
                GLOBAL_MAIN_VARS["SITE_URL"] +
                "workflow_pengajuan/workflow_pengajuan_restructure/saveAssignData",
              data: {
                id: $("#id").val(),
                tl: $("#team_leader").val(),
      ...
      showCommonDialog(
        ...
        "ASSIGNED",
        GLOBAL_MAIN_VARS["SITE_URL"] +
          "workflow_pengajuan/workflow_pengajuan_restructure/assignData?id=" +
          selected_data.id,
        buttons
      );
    } else {
  ...
  if (selected_data.status) {
    if (
      ...
      showCommonDialog(
        ...
        "Approval",
        GLOBAL_MAIN_VARS["SITE_URL"] +
          "workflow_pengajuan/workflow_pengajuan_restructure/approvalrequest?id=" +
          selected_data.id,
        buttons
      );
    }
